...
# ============================================================
#                    My Py Lib - SpotDL
#          Universal Installer (Diagnose + Debian/Native)
#
...
echo "================================================"
echo "     Universal Installer (Diagnose + Debian/Native)"
echo "              $CREDIT"
echo "================================================"
echo
...
# ------------------------------------------------------------
# Storage permission helper (needed before configuring save path)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Custom music save location
#
# Stored as a path RELATIVE to shared storage (e.g. "Music/SpotDL")
# in $HOME/.spotdl-music-relpath, so both install modes can resolve
# it to the correct absolute path for their environment:
#   Termux / native mode : $HOME/storage/shared/<relpath>
#   Debian mode           : /sdcard/<relpath>  (proot-distro binds
#                            /sdcard into the Debian rootfs by default,
#                            so downloads land directly on your phone
#                            storage with no manual move step needed)
# ------------------------------------------------------------

MUSIC_RELPATH_FILE="$HOME/.spotdl-music-relpath"

configure_music_dir() {
    local default_rel="Music/SpotDL"
    local current_rel=""
    [ -f "$MUSIC_RELPATH_FILE" ] && current_rel="$(cat "$MUSIC_RELPATH_FILE")"
    [ -n "$current_rel" ] && default_rel="$current_rel"

    echo
    echo "Where should downloaded songs be saved?"
    echo "Enter a path relative to your phone's shared storage."
    echo "Example: Music/SpotDL   or   Download/Music"
    if [ -n "$current_rel" ]; then
        echo "Current setting: $current_rel"
    fi
    read -p "Save location [$default_rel]: " input_rel

    local rel="${input_rel:-$default_rel}"
    rel="${rel#/}"
    rel="${rel%/}"
    [ -z "$rel" ] && rel="$default_rel"

    echo "$rel" > "$MUSIC_RELPATH_FILE"

    if [ -d "$HOME/storage/shared" ]; then
        if mkdir -p "$HOME/storage/shared/$rel" 2>/dev/null; then
            ok "Music will be saved to: $HOME/storage/shared/$rel"
        else
            warn "Could not create that folder - check the path is valid."
        fi
    else
        warn "Storage isn't linked yet, so the folder can't be created right now."
        warn "It will be created automatically on first download once storage is available."
    fi
}

# Standalone flag to just change the save location without reinstalling
# anything: bash install.sh --set-music-dir
for arg in "$@"; do
    if [ "$arg" = "--set-music-dir" ]; then
        ensure_storage_permission
        configure_music_dir
        echo
        ok "Save location updated. No reinstall needed - the spotdl-debian command and widget already read this setting automatically."
        exit 0
    fi
done
...
# -1 is a sentinel meaning "could not detect" so it never gets
# treated as "0MB free" (which previously forced NATIVE mode
# incorrectly on devices where df just didn't parse cleanly).
FREE_STORAGE_MB=-1
if command -v df >/dev/null 2>&1; then
    _df_val="$(df -Pm "$HOME" 2>/dev/null | awk 'NR==2 {print $4}')"
    case "$_df_val" in
        ''|*[!0-9]*) FREE_STORAGE_MB=-1 ;;
        *) FREE_STORAGE_MB="$_df_val" ;;
    esac
fi
# Fallback if df gave nothing usable: try stat on the filesystem
if [ "$FREE_STORAGE_MB" -lt 0 ] && command -v stat >/dev/null 2>&1; then
    _blocks_free="$(stat -f -c '%a' "$HOME" 2>/dev/null)"
    _block_size="$(stat -f -c '%S' "$HOME" 2>/dev/null)"
    if [ -n "$_blocks_free" ] && [ -n "$_block_size" ]; then
        case "$_blocks_free$_block_size" in
            *[!0-9]*) : ;;
            *) FREE_STORAGE_MB=$(( _blocks_free * _block_size / 1024 / 1024 )) ;;
        esac
    fi
fi
...
if [ "$FREE_STORAGE_MB" -lt 0 ]; then
    printf "%-22s %s\n" "Free storage:" "could not detect"
else
...
if [ "$DEBIAN_ARCH_OK" -eq 0 ]; then
    INSTALL_MODE="native"
    add_reason "architecture '$ARCH' has no proot-distro Debian build"
fi

if [ "$FREE_STORAGE_MB" -ge 0 ] && [ "$FREE_STORAGE_MB" -lt 1200 ]; then
    INSTALL_MODE="native"
    add_reason "free storage is low (${FREE_STORAGE_MB}MB, Debian needs ~1.2GB+)"
fi

if [ "$TOTAL_RAM_MB" -gt 0 ] && [ "$TOTAL_RAM_MB" -lt 1024 ]; then
    INSTALL_MODE="native"
    add_reason "device RAM is very low (${TOTAL_RAM_MB}MB)"
fi
...
for arg in "$@"; do
    case "$arg" in
        --native)
            INSTALL_MODE="native"
            REASON_TEXT="
  - forced by --native flag"
            ;;
        --debian)
            if [ "$DEBIAN_ARCH_OK" -eq 1 ]; then
                INSTALL_MODE="debian"
                REASON_TEXT="
  - forced by --debian flag"
            else
...
if [ "$INSTALL_MODE" = "native" ]; then
    ...
    if [ -n "$REASON_TEXT" ]; then
        echo "$REASON_TEXT" | while IFS= read -r line; do
            [ -n "$line" ] && warn "$line"
        done
    fi
else
...
step "[SETUP] Choosing where to save downloaded music..."
configure_music_dir
...
rm -f /root/.spotdl-setup-ok

# apt/dpkg can be locked by an unrelated background process (e.g. an
# auto-update). Retry instead of failing on the first hit.
apt_retry() {
    local n=0
    local max=12
    until apt-get "\$@"; do
        n=\$((n+1))
        if [ "\$n" -ge "\$max" ]; then
            echo "apt-get \$* failed after \$max attempts (lock never freed or a real error)."
            return 1
        fi
        echo "apt/dpkg appears locked or the command failed - retrying in 8s (attempt \$n/\$max)..."
        sleep 8
    done
}
...
if [ -n "\$NEEDED_PKGS" ]; then
    ...
    apt_retry install -y \$NEEDED_PKGS
else
...
# Written only if every step above actually succeeded - this is what
# the outer Termux script checks, since proot-distro's own exit code
# is not always reliable.
touch /root/.spotdl-setup-ok
DEBIAN_SETUP
    ...
    if ! proot-distro login debian -- bash -lc 'test -f "$HOME/.spotdl-setup-ok"'; then
        fail "Debian-side setup did not complete successfully (see the errors above)."
        fail "Common cause: apt/dpkg was locked by another process - close other Termux/Debian sessions and re-run this installer."
        exit 1
    fi

    proot-distro login debian -- bash -lc 'rm -f "$HOME/.spotdl-setup-ok"' >/dev/null 2>&1 || true
...
RELPATH_FILE="\$HOME/.spotdl-music-relpath"
RELPATH="Music/SpotDL"
[ -f "\$RELPATH_FILE" ] && RELPATH="\$(cat "\$RELPATH_FILE")"
...
if [ "\$MODE" = "native" ]; then
    ...
    TARGET_DIR="\$HOME/storage/shared/\$RELPATH"
...
DOWNLOAD_DIR="\$HOME/storage/shared/\$RELPATH"
mkdir -p "\$DOWNLOAD_DIR"

echo "Mode: \$MODE"
echo "Downloads will be saved to:"
echo "\$DOWNLOAD_DIR"
echo "(change this any time with: bash install.sh --set-music-dir)"
echo
...
if [ "$FREE_STORAGE_MB" -lt 0 ]; then
    echo "Free storage:   could not detect"
else
    echo "Free storage:   ${FREE_STORAGE_MB}MB"
fi
echo
CURRENT_REL="Music/SpotDL"
[ -f "$MUSIC_RELPATH_FILE" ] && CURRENT_REL="$(cat "$MUSIC_RELPATH_FILE")"
echo "Music save location:  storage/shared/$CURRENT_REL"
echo
...
echo
echo "Change where music is saved any time with:"
echo "  bash install.sh --set-music-dir"
echo
